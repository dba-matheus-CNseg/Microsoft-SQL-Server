...
SELECT  
   ...
   ROUND(avg_fragmentation_in_percent, 2)AS [fragmentation],
	 script = case
        when dm.avg_fragmentation_in_percent > 30 then "ALTER INDEX ["+ i.name +"] ON ["+ db_name() +"].["+ s.name +"].["+ t.name +"] REBUILD
		GO"
        when dm.avg_fragmentation_in_percent >= 5 and avg_fragmentation_in_percent <= 30 then "ALTER INDEX ["+ i.name +"] ON ["+ db_name() +"].["+ s.name +"].["+ t.name +"] REORGANIZE
		GO"
    end
FROM    
    sys.tables t    
